...
@Component({
      ...
      @if (loading()) {
        ...
        <div class="event-detail">
          ...
          <div class="event-detail__body">
            <div class="info-grid">
              <div class="info-item">
                <span class="info-item__label">📅 Start</span>
                <span>{{ event()!.startDateTime | date:'full' }}</span>
              </div>
              <div class="info-item">
                <span class="info-item__label">🏁 End</span>
                <span>{{ event()!.endDateTime | date:'full' }}</span>
              </div>
              @if (event()!.location) {
                <div class="info-item">
              ...
              <div class="info-item">
                <span class="info-item__label">🔒 Privacy</span>
                <span>{{ event()!.privacy }}</span>
              </div>
              @if (event()!.maxAttendees > 0) {
                <div class="info-item">
                  <span class="info-item__label">👥 Max Attendees</span>
                  <span>{{ event()!.maxAttendees }}</span>
                </div>
              }
              <div class="info-item">
                <span class="info-item__label">🎫 Tickets Sold</span>
  ...
  `,
    ...
    .event-detail { background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.06); }
    .event-detail__header { padding: 2rem; border-left: 6px solid #3498db; display: flex; justify-content: space-between; align-items: flex-start; background: #f8f9fa; }
    .event-detail__header h1 { font-size: 1.75rem; color: #2c3e50; margin: 0.5rem 0; }
    .organizer { color: #7f8c8d; font-size: 0.9rem; }
    ...
    .info-item__label { font-size: 0.8rem; font-weight: 600; color: #7f8c8d; text-transform: uppercase; letter-spacing: 0.5px; }
    .info-item span:last-child { font-size: 0.95rem; color: #2c3e50; }
    .description h3 { font-size: 1rem; color: #2c3e50; margin-bottom: 0.75rem; }

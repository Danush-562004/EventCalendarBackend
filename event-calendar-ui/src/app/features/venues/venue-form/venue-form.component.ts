            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()">
                <div class="form-grid">
                    ...
                    <div class="form-group form-group--full">
                        ...
                        <textarea formControlName="description" rows="3" placeholder="Venue description"></textarea>
                    </div>
                </div>
                <div class="form-actions">
                    <a routerLink="/venues" class="btn btn--outline">Cancel</a>
                    <button type="submit" class="btn btn--primary" [disabled]="loading()">
                        {{ loading() ? 'Saving...' : (isEdit ? 'Update Venue' : 'Add Venue') }}
                    </button>
                </div>
            </form>
        </div>
...
export class VenueFormComponent implements OnInit {
    ...
    onSubmit() {
        ...
        obs.subscribe({
            next: () => {
                this.toast.success(this.isEdit ? 'Venue updated.' : 'Venue created.');
                this.router.navigate(['/venues']);
            },
            error: err => {
                this.toast.error(err.error?.message || 'Failed to save venue.');

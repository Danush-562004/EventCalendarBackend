...
@Component({
    ...
    template: `
    <div class="page">
      <div class="page__header"><h1>My Tickets</h1></div>
      @if (loading()) {
        <div class="loading">Loading tickets...</div>
      } @else if (tickets().length === 0) {
        <div class="empty-state"><span>🎫</span><p>No tickets yet. Book tickets for events you're interested in!</p></div>
      } @else {
          ...
          @for (ticket of tickets(); track ticket.id) {
            <div class="ticket-card" [class.ticket-card--cancelled]="ticket.status === 'Cancelled'">
              ...
              <div class="ticket-card__checkin">
                @if (ticket.checkedIn) { <span class="checkin-badge">✓ Checked In</span> }
              </div>
            </div>
          }
        </div>
  ...
  `,
    ...
    .ticket-card { display: flex; align-items: center; gap: 1.5rem; background: var(--card); border-radius: 12px; padding: 1.25rem 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border-left: 4px solid var(--primary); }
    .ticket-card--cancelled { opacity: 0.6; border-left-color: var(--danger); }
    .ticket-card__number { font-family: monospace; font-size: 0.8rem; color: var(--muted); background: var(--bg); padding: 0.4rem 0.75rem; border-radius: 4px; white-space: nowrap; }
    .ticket-card__body { flex: 1; }
    .ticket-card__body h3 { font-size: 1rem; color: var(--dark); margin-bottom: 0.5rem; }
    .ticket-card__meta { display: flex; align-items: center; gap: 0.75rem; flex-wrap: wrap; margin-bottom: 0.5rem; font-size: 0.85rem; color: var(--muted); }
    ...
    .type-badge { background: var(--bg); padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.75rem; }
    .ticket-card__price { font-size: 1.1rem; font-weight: 700; color: var(--dark); }
    .checkin-badge { background: #dcfce7; color: var(--success); padding: 0.3rem 0.75rem; border-radius: 20px; font-size: 0.8rem; font-weight: 600; }
  `]
})

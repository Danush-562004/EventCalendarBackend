...
@Component({
          ...
          @if (!isEdit) {
            <div class="form-group">
              ...
              @if (form.get('eventId')?.invalid && form.get('eventId')?.touched) {
                <span class="form-error">Please select an event.</span>
              }
            </div>
          }
  ...
  `,
    ...
    .form-card { background: #fff; border-radius: 12px; padding: 2rem; box-shadow: 0 2px 12px rgba(0,0,0,0.06); display: flex; flex-direction: column; gap: 1.25rem; }
    .form-group { display: flex; flex-direction: column; gap: 0.4rem; }
    .form-group label { font-size: 0.875rem; font-weight: 600; color: #2c3e50; }
    .form-group input, .form-group select, .form-group textarea {
      padding: 0.75rem 1rem; border: 1.5px solid #dde1e7; border-radius: 8px; font-size: 0.95rem; outline: none; font-family: inherit;
    }
    ...
    .form-actions { display: flex; justify-content: flex-end; gap: 1rem; padding-top: 1rem; border-top: 1px solid #f0f0f0; }
...
export class ReminderFormComponent implements OnInit {
        ...
        if (this.reminderId) {
            ...
            this.reminderService.getById(this.reminderId).subscribe(res => {
                ...
                this.form.patchValue({
                    title: r.title,
                    message: r.message,
                    reminderDateTime: r.reminderDateTime.slice(0, 16)
                });
            });
        }
    }
    ...
    onSubmit() {
        ...
        obs.subscribe({
            next: () => {
                this.toast.success(this.isEdit ? 'Reminder updated.' : 'Reminder created.');
                this.router.navigate(['/reminders']);
            },
            error: err => {
                this.toast.error(err.error?.message || 'Failed to save reminder.');
